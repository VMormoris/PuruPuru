...
namespace gui {
		...
		ImVec2 rectMin, ImVec2 rectMax)
	{
		...
		const ImU32 color = GetButtonColor(tintNormal, tintHovered, tintPressed);
		drawList->AddImage(id, rectMin, rectMax, { 0.0f, 0.0f }, { 1.0f, 1.0f }, color);
	};
		...
		ImRect rectangle)
	{
		auto* drawList = ImGui::GetForegroundDrawList();
		const ImU32 color = GetButtonColor(tintNormal, tintHovered, tintPressed);
		drawList->AddImage(id, rectangle.Min, rectangle.Max, { 0.0f, 0.0f }, { 1.0f, 1.0f }, color);
	};

	void DrawButtonImage(const ImTextureID id, ImU32 tintNormal, ImU32 tintHovered, ImU32 tintPressed)
	{
		auto* drawList = ImGui::GetForegroundDrawList();
		const ImU32 color = GetButtonColor(tintNormal, tintHovered, tintPressed);
		drawList->AddImage(id, ImGui::GetItemRectMin(), ImGui::GetItemRectMax(), { 0.0f, 0.0f }, { 1.0f, 1.0f }, color);
	};
	...
	void DrawTitlebar(float& outTitlebarHeight)
	{
		...
		const bool titleBarHovered = ImGui::IsItemHovered();
...

		{
			...
			ImVec2 textSize = ImGui::CalcTextSize("PuruPuru v0.2.0");
			ImGui::SetCursorPos(ImVec2(ImGui::GetWindowWidth() * 0.5f - textSize.x * 0.5f, 2.0f + windowPadding.y + 6.0f));
			ImGui::Text("%s", "PuruPuru v0.2.0"); // Draw title
		...
		//ShiftCursorY(8.0f);
		{
			...
			DrawButtonImage(sMinimize.GetHandle(), buttonColN, buttonColH, buttonColP, RectExpanded(GetItemRect(), { 0.0f, -padY }));
		}
...
static ImU32 GetButtonColor(ImU32 tintNormal, ImU32 tintHovered, ImU32 tintPressed)
{
	if (ImGui::IsItemActive()) return tintPressed;
	else if (ImGui::IsItemHovered()) return tintHovered;
	else return tintNormal;
}